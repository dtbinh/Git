...
if optargin > 0
    for j =1:optargin
        ...
        if(~isstruct(my_string{1}))
            ...
            elseif strfind(my_string{1},'lab')
                   label = cell2mat(varargin(1,j+1));              
            end
        end
    end
end
...
if norm(dq) ~= 1
    if(D(dq)==0 && Re(P(dq))==0)
        dq_t = Im(P(dq));
        handle.handle_axis{1} = plot3(dq_t.q(2),dq_t.q(3),dq_t.q(4), 'r.','MarkerSize',10,'erasemode', 'xor');
        handle.handle_text{1} = text(dq_t.q(2), dq_t.q(3), dq_t.q(4), ['' label]);
        set(handle.handle_text{1} , 'Color', 'k');
    else
        error('Only unit dual quaternions or points can be plotted');
    end
else
	...
	handle.handle_text{1} = text(dq_t.q(2)+x_t.q(2), dq_t.q(3)+x_t.q(3), dq_t.q(4)+x_t.q(4), ['x' label]);
	set(handle.handle_text{1} , 'Color', 'k');

	handle.handle_axis{2} =plot3([0;y_t.q(2)]+dq_t.q(2), [0; y_t.q(3)]+dq_t.q(3), [0; y_t.q(4)]+dq_t.q(4), 'g','Linewidth',1,'erasemode', 'xor');
	handle.handle_text{2}  = text(dq_t.q(2)+y_t.q(2), dq_t.q(3)+y_t.q(3), dq_t.q(4)+y_t.q(4), ['y' label]);
	...
	handle.handle_text{3}  = text(dq_t.q(2)+z_t.q(2), dq_t.q(3)+z_t.q(3), dq_t.q(4)+z_t.q(4), ['z' label]);
