function [policy Q] = SARSA(epsilon, alpha, maxEpisode)

% SARSA Applies the SARSA RL algorithm to the Windy Gridworld problem
%    [P Q] = SARSA(E,A,N) applies the SARSA temporal difference method to 
%    the Reinforcement Learning problem of moving in the Windy Gridworld.
%    The problem consists in learning the best policy P and the associated
...
%
%    See also: MOTECARLO, QLEARNING, INITVALUEFUNCTION, INITPOLICY, NEXTSTATE
...
for iEpisode = 1:maxEpisode
    ...
    while(~(x == xGoal && y == yGoal))
        ...
        % Randomly select between following the policy or not
        if(rand > epsilon)
            newA = policy(newX, newY);
        else
            newA = randomValidAction(newX, newY, xMax, yMax);
        end
        
        % Upadte the value function
        Q(x,y,a) = Q(x,y,a) + alpha*(reward + Q(newX,newY,newA) - Q(x,y,a));
        iStep = iStep + 1;
